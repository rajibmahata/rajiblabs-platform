  ...
  return (
    <div className="min-h-screen flex items-center justify-center p-6" style={{ background: "var(--c-bg-primary)" }}>
      <form onSubmit={submit} className="w-full max-w-md p-8 rounded-2xl border" style={{ background: "var(--c-bg-secondary)", borderColor: "var(--c-border)" }}>
        ...
        <label className="block text-xs mb-1" style={{ color: "var(--c-text-muted)", fontFamily: "JetBrains Mono, monospace" }}>USERNAME</label>
        <input value={u} onChange={e => setU(e.target.value)} required className="w-full px-3 py-2 rounded-lg border mb-4" style={{ background: "var(--c-bg-tertiary)", borderColor: "var(--c-border)", color: "var(--c-text-primary)" }} placeholder="admin" />
        ...
        <p className="text-xs mt-4 text-center" style={{ color: "var(--c-text-muted)" }}>Default: admin / Admin@123 — change via `Admin__Password` env.</p>
      </form>
    </div>
  );
}
