...
const fallbackProjects: ProjectDetail[] = [
  ...
  {
    name: 'LexVault',
    shortDesc: 'Legal document intelligence platform — LLM-assisted knowledge base ingestion + zero-LLM confidence scoring via hybrid dense+sparse search on Qdrant. On-premise Windows Server, no cloud dependency.',
    longDesc: 'LexVault is a Legal Document Intelligence Platform built on a dual-pipeline architecture. Pipeline 1 (Ingestion): legal documents are parsed, chunked with legal-aware boundary detection, embedded via Azure OpenAI, and indexed into Qdrant with dense + sparse BM42 vectors. GPT-4o extracts clause types, legal entities, and risk signals once — all stored as Qdrant payload and never regenerated. Pipeline 2 (Scoring): uploaded documents are scored without any LLM calls — the system uses Qdrant hybrid search (HNSW dense ANN + BM42 sparse keyword matching) fused via Reciprocal Rank Fusion, plus a custom .NET Legal Clause Match Engine (regex + keyword patterns for 20+ clause types). Three independent signals — dense semantic similarity (40%), sparse keyword match (35%), and clause hit rate (25%) — combine into a weighted confidence score with per-clause breakdown and missing-clause flagging. The entire stack runs on Windows Server 2022 with IIS, Qdrant as a Windows Service, Memurai for Redis, Hangfire on SQL Server for job queues, and win-acme for Let\'s Encrypt SSL — zero cloud infrastructure costs.',
    features: ['Dual-pipeline: LLM ingestion + zero-LLM scoring', 'Qdrant hybrid search — dense HNSW + sparse BM42 + RRF fusion', 'Legal Clause Match Engine — 20+ clause patterns (regex + keyword)', 'Three-signal weighted confidence score (0–100) with per-clause breakdown', 'Legal-aware chunking — clause boundary detection, hierarchy preservation', 'On-premise Windows Server 2022 — IIS, Qdrant, Memurai, Hangfire, win-acme', 'Optional local GPU embedding model — eliminates all per-query API cost', 'Incremental KB updates — new documents indexed without full re-ingestion'],
    techStack: ['.NET 8', 'Qdrant', 'Hybrid Search', 'RAG', 'Azure OpenAI', 'Redis', 'SQL Server'],
    githubUrl: 'https://github.com/rajibmahata/Legal-Document-RAG-System-LEXVAULT', source: 'github', status: 'wip',
    role: 'Solution Architect & .NET Engineer — RajibLabs',
    impact: 'Zero-LLM-at-inference legal document scoring. On-premise architecture eliminates all cloud infrastructure costs. Designed for Windows Server with no Docker or Linux dependency.',
  },
  {
    name: 'AI Student Tutor',
    shortDesc: 'Multi-role AI-powered learning platform with 12 specialised agents — voice-first tutoring, auto content generation, gamification, and human-in-the-loop validation. 4 languages, Nursery to Class 12.',
    longDesc: 'AI Student Tutor is a comprehensive educational platform that unifies five participant groups — AI, Human Tutors, Principals, Administrators, and Students — into a single personalized learning ecosystem. At its core is a 12-agent AI architecture: Teacher Agent (voice-first conversational tutoring with Hint → Guide → Solve progression), Assessment Agent (MCQ + subjective + image-OCR evaluation), Curriculum Agent (Subject → Chapter → Topic knowledge graph), Motivation Agent (streaks, badges, points, leaderboards), Content Generation Agent (PDF → structured lessons + summaries + exercises), Personalization Agent (adapts by language, region, culture, grade, and interests), Video Generation Agent (auto-creates animated concept explanations), Verification Agent (auto-verifies tutor qualifications and documents), Voice Agent (STT/TTS in 4+ languages with regional accents), Matching Agent (intelligent tutor ↔ student pairing on 4 criteria), Analytics Agent (learning patterns, trends, predictions), and Report Agent (multi-role reports for Student/Tutor/Principal/Admin). The platform follows a structured 8-step learning journey: Subject Selection → Personalized Content Delivery → Voice-Based Learning → Chapter Learning → Mock Test → Personalized Revision → Final Assessment → Progress Tracking. A critical design choice is the Human-in-the-Loop model: all AI-generated content must be approved by a qualified tutor before reaching students. The platform supports 4 roles — Student, Tutor, Principal, and Super Admin — each with purpose-built dashboards, notification centers, and approval workflows. Built with Next.js 15 + TypeScript frontend, FastAPI + LangGraph backend, PostgreSQL, Redis, Qdrant vector DB, MinIO for file storage, and RabbitMQ for async messaging.',
    features: ['12 specialised AI agents — Teacher, Assessment, Content Gen, Voice, Analytics & more', 'Voice-first conversational tutoring — Hint → Guide → Solve progression', 'Structured 8-step learning journey from Subject Selection to Progress Tracking', 'AI content generation: PDF → lessons + videos + quizzes (human-validated)', 'Personalization along 4 dimensions: Language, Region, Culture, Grade', '4 role-based portals: Student, Tutor, Principal, Super Admin', 'Intelligent tutor↔student matching on academic, language, regional & cultural criteria', 'Subjective answer evaluation via image upload + AI OCR + tutor review'],
    techStack: ['FastAPI', 'LangGraph', 'Next.js 15', 'TypeScript', 'PostgreSQL', 'Qdrant', 'Redis', 'MinIO', 'RabbitMQ', 'OpenAI'],
    githubUrl: 'https://github.com/rajibmahata/Math-tutor-AI-Agent', source: 'github', status: 'wip',
    role: 'AI Architect & Full-Stack Developer — RajibLabs',
    impact: '12-agent AI platform unifying students, tutors, principals & admins. Designed to scale from individual tutoring to institution-wide deployment with full governance hierarchy.',
  },
