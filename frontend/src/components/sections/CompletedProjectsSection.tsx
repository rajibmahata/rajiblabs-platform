...
export const projects: ProjectDetail[] = [
  ...
  {
    name: 'MedRemind',
    shortDesc: 'AI-powered medication reminder app with two-stage OCR pipeline supporting 10 languages including RTL.',
    longDesc: "Medication non-adherence is one of the most costly and preventable problems in healthcare. MedRemind is an AI-first solution: point your phone camera at a prescription label, and the app does the rest. The two-stage OCR pipeline is the technical core — Stage one uses Azure Document Intelligence to extract raw text from the prescription label image, handling varied fonts, print quality, and label formats. Stage two passes the extracted text to GPT-4o-mini with a structured prompt that parses the medication name, dosage, frequency, and duration into a structured JSON schedule. This two-stage approach is significantly more accurate than single-model OCR on real-world prescription labels. Once the schedule is parsed, the app creates reminder entries and schedules notifications through the device's native notification system. The reminder text is localised into the patient's preferred language — the app supports 10 languages covering major Indian and global markets, with full RTL support for Arabic and Urdu.",
    features: ['Two-stage OCR: Azure Document Intelligence → GPT-4o-mini', '10-language support with full RTL (Arabic, Urdu)', 'Cross-platform iOS + Android from single React Native codebase', 'Structured JSON extraction of medication details', 'Native push notification scheduling', 'FastAPI backend for AI pipeline & schedule storage'],
    techStack: ['React Native', 'Expo', 'Python', 'FastAPI', 'Azure Document Intelligence', 'GPT-4o-mini', 'SQL Server'],
    githubUrl: 'https://github.com/rajibmahata/MedRemind',
    source: 'github', status: 'complete',
    role: 'AI Engineer & Full-Stack Developer',
    impact: 'Two-stage OCR achieves higher accuracy than single-model approaches on real-world prescription labels. Demonstrated AI integration that delivers genuine workflow improvement — not a chatbot, but a measurable accuracy gain for patient safety.',
  },
  {
    name: 'ArtForge',
    shortDesc: 'Agentic drawing portfolio with multi-agent AI validation — Duplicate Detector, Quality Validator & Originality Scorer.',
    longDesc: 'ArtForge solves a problem every creative portfolio platform faces: how do you maintain quality at scale without human curation? The answer is a multi-agent validation pipeline. When an artist submits a drawing, it enters the pipeline: the Duplicate Detector uses vector embedding similarity search (RAG) to check for duplicates and near-duplicates; the Quality Validator analyses the image for technical quality indicators like resolution, composition, and linework consistency; the Originality Scorer assesses whether the style is derivative or genuinely novel. Only artwork that passes all three stages is published. Artists whose work is rejected receive specific feedback from the relevant agent. The platform uses the Claude API for reasoning in the Quality Validator and Originality Scorer stages, with RAG vector embeddings stored in ChromaDB. ArtForge is a proof-of-concept for the RajibLabs multi-agent architecture — demonstrating that specialised validation pipelines can be applied to any domain requiring quality assurance at scale.',
    features: ['Three-agent validation: Duplicate Detector, Quality Validator, Originality Scorer', 'RAG-based vector embedding similarity search', 'Automated rejection with specific agent-level feedback', 'Image analysis for technical quality indicators', 'Claude API reasoning for Quality & Originality scoring', 'Proof-of-concept for multi-agent QA at scale'],
    techStack: ['React 18', 'Python', 'FastAPI', 'Claude API', 'ChromaDB', 'RAG', 'Vector Embeddings'],
    githubUrl: 'https://github.com/rajibmahata/ArtForge',
    source: 'github', status: 'complete',
    role: 'AI Architect & Full-Stack Developer',
    impact: 'Demonstrates multi-agent AI validation pipelines applicable to any domain requiring quality assurance at scale. Three specialised agents replace human curation while providing specific, actionable feedback.',
  },
  {
    name: 'AIResumeReviewer',
    shortDesc: 'Seven-agent AI career platform — ATS scan, keyword gap, achievement rewriting, industry calibration & full resume rewrite.',
    longDesc: 'Most AI resume tools give generic feedback: "Add more keywords. Use action verbs." AIResumeReviewer does better. The seven-agent pipeline breaks review into specialised functions: ATS Scanner checks tracking system filters; Keyword Gap Agent identifies missing skills against the job description; Achievement Phrasing Agent rewrites bullets using the XYZ formula; Formatting Agent flags structural issues; Industry Calibration Agent benchmarks against sector norms; Role Matching Agent scores overall fit; Rewrite Agent produces a complete revised draft. The RAG system uses a corpus of anonymised high-quality resume examples to ground recommendations — feedback is based on what actually works, not generic rules. Built on .NET 8 Minimal API, React 18, and Python FastAPI, the seven agents run as a configurable sequential pipeline with a shared context object.',
    features: ['Seven specialised AI agents for distinct review functions', 'ATS compatibility scanner for target role filtering', 'Keyword gap analysis against job descriptions', 'XYZ formula achievement bullet rewriting', 'Industry-specific calibration benchmarking', 'Full revised resume draft as pipeline output'],
    techStack: ['.NET 8', 'React 18', 'Python', 'FastAPI', 'GPT-4o', 'ChromaDB', 'RAG'],
    githubUrl: 'https://github.com/rajibmahata/AIResumeReviewer',
    source: 'claude_cb', status: 'complete',
    role: 'AI Engineer & Full-Stack Developer',
    impact: 'Seven-agent pipeline provides depth no generic tool can match. RAG-grounded recommendations based on real successful resume patterns. Produces a complete revised draft, not just bullet-point suggestions.',
  },
  {
    name: 'AgentTube (SahajSeva)',
    shortDesc: 'AI-powered PWA for Annapurna Yojana welfare forms — OCR + conversational AI in Bengali, Hindi & English.',
    longDesc: 'The Annapurna Yojana scheme provides food security to eligible Indian households, but application forms present a barrier to the people who need them most — those with low digital literacy. AgentTube is a conversational form assistant. Users open the PWA (no app store required, works on low-end Android), select their language (Bengali, Hindi, or English), and the app guides them through each form field. Users can photograph existing identity documents — GPT-4o OCR extracts relevant information to pre-fill fields. The DeepSeek V3 Q&A layer answers questions conversationally in the user\'s language. The conversational design means users with no digital experience can complete applications without help. Tech stack chosen for cost-effectiveness and regional accessibility. This project demonstrates AI\'s potential for social impact — making government services accessible without intermediaries.',
    features: ['Three-language support: Bengali, Hindi, English', 'GPT-4o document OCR extracts info from photographed IDs', 'DeepSeek V3 conversational Q&A for eligibility questions', 'PWA — installable from browser, works on low-end Android', 'Step-by-step guided form completion', 'Designed for citizens with low digital literacy'],
    techStack: ['React', 'PWA', 'GPT-4o', 'DeepSeek V3', '.NET 8', 'SQL Server'],
    githubUrl: 'https://github.com/rajibmahata/AgentTube',
    source: 'github', status: 'wip',
    role: 'AI Engineer & Social Impact Developer',
    impact: 'Social impact project demonstrating AI\'s potential for accessibility. PWA design ensures reach on low-end devices. Three-language support removes language barriers for government welfare access.',
  },
  {
