...
async def consolidate_resume_projects(db=None, triggered_by: str = "profile_agent") -> dict:
    ...
    if db is None:
        db = get_db()
    stats = {"resumes_scanned": 0, "projects_found": 0, "deduped": 0, "created": 0, "updated": 0, "skipped": 0, "errors": []}
    ...
    # Also portfolio slugs to avoid duplicate creation for already existing portfolio items that will be served via same slug path
    portfolio_slugs = {d.get("slug","").lower() async for d in db["portfolio"].find({}, {"slug":1})}
    # GitHub repos for enrichment (verified URLs)
    github_by_name = {}
    ...
    for slug, proj in deduped.items():
        ...
        if existing:
            ...
            # No live/github URL invention: only if resume had explicit URL (rare) we could add, but resume text typically has none
            # Enrich with verified GitHub URL if exists and missing
            if not existing.get("github_url") and github_by_name.get(low_slug):
                repo = github_by_name[low_slug]
                patch["github_url"] = repo.get("html_url")
            # Also check github by slug in pestflow case? already
            if patch:
            ...
            # Find github_url if verified repo exists
            github_url = proj.get("github_url")
            if not github_url and github_by_name.get(low_slug):
                github_url = github_by_name[low_slug].get("html_url")
    ...
    try:
        await audit("profile_agent", "RESUME_PROJECTS_CONSOLIDATED", triggered_by, {"created": stats["created"], "updated": stats["updated"], "found": stats["deduped"]})
    except Exception:
        pass
    return stats
